...
### Step 2: V39.1.1 본실험 (iter 4500 조기 중단)

Run `2026-03-23_22-14-10`:

| iter | shoulder dev | phase | ep_len | stride | FL/FR | splay% |
|------|-------------|-------|--------|--------|-------|--------|
| 1000 | 0.520 | 4.88 | 249 | 5.22 | 0.85/0.84 | 3.2% |
| 2000 | 0.486 | 4.89 | 227 | 5.36 | 0.81/0.80 | 17.7% |
| 3000 | 0.447 | 5.13 | 216 | 4.94 | 0.75/0.74 | 30.2% |
| 4000 | 0.438 | 5.29 | 212 | 4.40 | 0.74/0.74 | 30.5% |

**결과**: V38.3(0.45) 대비 shoulder dev 0.438 (-0.012 미미). stride 7.23→4.40 (-39% 악화).
**결론**: CPG가 splay를 해결하지 못하고 stride만 악화시킴. 조기 중단.

---

## 4. Reward 구조 분석 (V39.1.1 iter 4000 기준)

### 0.45 장벽의 원인: 물리적 손익분기점

V38.3에서 shoulder dev 감소 시 reward 변화를 추적한 결과:

| 구간 | shoulder dev | contact_band 변화 | stance_w 변화 | 순이익 |
|------|-------------|------------------|--------------|--------|
| 0.54→0.45 | -0.09 | -1.92 (손실) | +4.18 (이득) | **+4.17** |
| 0.45→0.45 (정체) | 0 | -0.45 | -1.06 | **-1.51** |
...
| 가설 | 결과 |
|------|------|
| "reward 불균형이 splay 원인" | **부분 맞음** — 0.54→0.45 구간에서는 CaT로 극복 가능 |
| "splay는 gait 부산물" (CPG 가설) | **틀림** — CPG로 gait 개선해도 splay 미변화 |
| "reward 조정으로 0.45 이하 가능" | **어려움** — 물리적 접지 한계가 근본 원인 |
