...
> **서기 정책을 살짝 바꿔 걷게 만드는 대신, 보행을 처음부터 다시 배우게 하자.**

그런데 V60은 단순히 "발을 들게 만들면 된다"는 문제가 아니었다.
실제로는 아래 local optimum들을 순서대로 만났다.

1. RR(오른뒤) 하나만 거의 안 쓰는 비대칭 exploit
2. 그 exploit을 막자 네 발 다 거의 붙여 버티는 정적 접지 해
3. 정적 해를 깨려 하자 앞다리만 swing
4. rear를 직접 밀자 뒷다리만 swing
5. front/rear 균형은 맞췄지만 diagonal 교대는 끝내 0

즉 V60의 핵심은
**"보행 reward를 주면 걷는다"가 아니라, 잘못된 해를 하나씩 막으면서 점점 gait 구조에 가까워지는 탐색 과정**이었다.
...
2. **정적 해 파괴 단계**
   - `V60.D ~ V60.F`
   - 정적으로 버티는 해를 깨고 실제 swing을 만들려는 단계

3. **구조화 단계**
   - `V60.G ~ V60.H`
   - 네 다리가 모두 움직이는 상태를 diagonal alternation으로 정리하려는 단계

즉:
- `V60.C`는 exploit 제거 성공
- `V60.F`는 rear swing 생성 성공
- `V60.G`는 4발 균형 swing 성공
- `V60.H`는 diagonal structure 도전
으로 읽으면 된다.
...
는 [V60_PLAN.md](/mnt/d/project/spot_micro_rl/plan/V60_PLAN.md)에 정리돼 있다.
