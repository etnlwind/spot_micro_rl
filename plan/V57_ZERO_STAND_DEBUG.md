...
4족 로봇은 support polygon 안에 CoM이 있으면 테이블처럼 수동적으로 서 있을 수 있어야 함.
→ 서 있지 못하면 물리 설정에 문제가 있는 것.

---

## 배경
- V57.B1: stand-first bootstrap (걷기 전에 서기부터)
- 이전 세션에서 Codex가 FK calibration + 대칭 init pose 완성
- init_z=0.194, stiffness=16, damping=2, effort_limit=15
- zero_stand_probe로 테스트 → 로봇이 서 있지 못함 → 오늘 디버깅 시작

---

## 테스트 환경
- `scripts/utils/zero_stand_probe.py`: action=0을 매 step 보내고 joint/torque/contact 기록
...
**결과**:
```
step=1:   height=0.2030 (init 0.194 → 올라감!), ang_xy=3.79, toe_fz=ALL 0.0
...
**관찰**:
1. Step 1에서 height가 0.194→0.203으로 **상승** (9mm) — 뭔가 위로 밀어올림
...
**초기 분석**:
- height 상승 → 지면에서 밀어올림? → toe sphere가 지면에 관통?
...
**가설**: 후방 다리 토크가 15 Nm에서 포화되어 position hold 실패. effort를 올리면 해결?
...
step=50: terminated=37.5% (이전 6.25% → 6배 악화!)
step=70: terminated=68.8%
```

**결론**: **토크 포화가 원인이 아님!** 더 큰 토크 → 더 큰 반발 에너지 → 더 심한 진동 → 더 빨리 사망.
...
**가설**: 20mm 관통이 문제. init_z를 올려서 toe sphere가 지면 위에 있게 하면 해결?
...
**결과**:
```
step=1:  height=0.203 (0.22에서 17mm 하락), ang_xy=?? (앞 로그에서 미기록)
...
**관찰**:
- 50 step까지 생존 (이전 6.25% → 0%) — 관통 제거 효과 확실
- 하지만 결국 불안정해짐
- RL_toe만 간헐적 접촉 (fz=9.96 at step 30), FL/FR/RR = 0 계속

---

### Test 4: stiffness=40, damping=5, effort=25 (강한 PD)

**가설**: PD가 너무 약해서 position hold 실패. 더 강한 PD면 해결?
...
**결론**: 강한 spring이 접촉 impulse를 증폭. 더 강한 PD ≠ 더 안정적.
...
3. **Leg이 천천히 드리프트**: -0.70 → -0.92 (200 steps에 걸쳐)
...
**확인 결과**: default_joint_pos = init_pos, offset = init_pos. action=0 → target = init_pos **맞음**.

그런데도 foot torque가 +15.0 (target 방향과 반대!)

---

### Test 6: damping=10, effort=15, init_z=0.22 (강한 감쇠)

**가설**: 높은 damping으로 진동 억제. 느리지만 안정적?
...
2. 하지만 **천천히 주저앉음** (height 0.22 → 0.07)
...
step=10: height=0.252 (더 상승!), 거의 모든 관절 ±50 포화
step=20: terminated=75%!
step=30: terminated=100%!
```

**결론**: 강한 PD가 착지 impulse를 증폭하여 로봇이 튕겨나감. **최악의 결과.**

---

### Test 8: init_z=0.214, Kp=16, Kd=5, effort=15 (최종 조합)

**가설**: 올바른 init_z + Codex가 튜닝한 PD 파라미터 조합
...
**가설**:
1. `robot.data.applied_torque`가 DCMotor output이 아닌 다른 값?
2. DCMotor 내부에서 예상과 다른 계산? (joint_vel_target이 0이 아닐 수 있음)
3. PhysX joint drive가 stiffness=0에도 불구하고 자체 힘을 가함?
4. 부호 convention: URDF joint axis 방향 vs torque 보고 방향?
5. `applied_torque`가 actuator effort가 아닌 PhysX net joint torque?
...
**가설**: effort_limit이 부족해서 crouched equilibrium으로 내려감. 올리면?
**결과**: 더 나빠짐! step 300: 37.5% terminated (이전 0%)
- 높은 effort = 높은 반발 에너지 = 더 불안정
- **결론: DCMotor에서 effort 증가는 해결책이 아님**

### Test 15: 기둥형 (columnar) pose — leg=-0.35, foot=0.63
**가설**: Z자 다리를 수직에 가깝게 펴면 moment arm 감소 → 적은 토크로 지지
**FK 계산**: height=0.247m (+15%), toe x=0.000
**결과**: 훨씬 나빠짐! step 50에서 25% terminated
- leg=-0.35는 θ=0까지 0.35 rad밖에 없어서 착지 충격으로 쉽게 flip
- Z자형 (leg=-0.70)은 0까지 0.70 rad 버퍼 → 더 안정
- **결론: 이론적으로 맞지만 실제로는 flip zone proximity가 더 위험**
...
- 하지만 여전히 "귀뚜라미" (height 0.12, 다리 크게 벌어짐)

### Test 17: foot/toe inertia 10배 증가 (URDF)
**설정**: foot Iyy 0.0005→0.005, toe Iyy 1e-5→1e-4
**가설**: 저관성이 PD 과잉반응의 원인. 서보 로터 관성 추가
**결과**: step 1이 훨씬 안정 (ang_xy=0.10 vs 1.74), 하지만 height 더 낮아짐 (0.073 vs 0.102)
- 높은 관성 = 느린 PD 응답 = body weight에 더 밀림
- **결론: 관성 증가는 초기 안정성↑, 장기 지지력↓. 복원함**
...
### 배경: DCMotor의 구조적 한계
위의 Test 14~17에서 DCMotor 환경 내에서 다양한 시도를 함:
- effort 증가 → 불안정
- pose 변경 → flip 위험
- gain 분리 → 부분 개선
- inertia 변경 → 트레이드오프

모든 시도에서 height 0.10~0.12 이상 유지 불가. **DCMotor의 velocity-dependent saturation이 구조적 병목**.
...
**가설**: DCMotor와 동일한 토크 제한을 걸어도 standing이 유지되나?
...
### Codex 수정: "맞다. effort_limit=15 유지한 비교 실험은 타당"
...
2. **effort_limit 증가 ≠ 안정성 증가**: 더 큰 토크 = 더 큰 반발 에너지
...
7. **ImplicitActuator + effort_limit = 두 마리 토끼**: PhysX 안정성 + 토크 제한 현실성
