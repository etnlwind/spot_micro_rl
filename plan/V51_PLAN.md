...
```text
standing reward는 계속 주고 있지만
walking reward가 훨씬 커서
결국 낮은 자세로도 잘 가면 이득
```
...
```text
walking reward에 높이 조건을 걸면
낮은 자세로 걷는 전략이 손해가 된다
```

### 실제로 한 일

`soft height gate`를 도입했고, 그 과정에서 세 번의 시행착오가 있었다.

```text
- 항상 활성, 강한 weight -> 즉사
- 항상 활성, 약한 weight -> 효과 부족
- boot-gated + 강한 weight -> 가장 좋은 초기 결과
```

### 결과

```text
- 초반 front_lift 0.257까지 상승
- 당시 기준 역대 최고
- 하지만 장기적으로 다시 하락
```
...
`V51`은 처음으로 `초반에는 성공하지만 장기적으로는 다시 망가지는` 패턴을 명확히 보여줬다.
즉 다음 단계는 `소프트 게이트만으로는 막히지 않는 낮은 자세 전략`을 더 직접 차단하는 방향으로 넘어간다.
