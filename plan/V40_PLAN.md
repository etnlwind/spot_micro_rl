...
### V40-A 결과 (iter 4200, 진행 중)

Run: `2026-03-24_16-11-29`

| iter | shoulder dev | splay% | ep_len | stride | V38.3 dev | 비고 |
|------|-------------|--------|--------|--------|-----------|------|
| 800 | 0.537 | 0.0% | 250 | 6.05 | 0.537 | CaT 시작, V38.3 동일 |
| 1000 | 0.524 | 6.1% | 245 | 6.16 | 0.532 | V40-A -0.008 빠름 |
| 1500 | 0.484 | 19.8% | 226 | 6.49 | 0.512 | V40-A -0.028 빠름 |
| 2000 | 0.448 | 31.9% | 210 | 6.09 | 0.492 | **0.45 돌파** |
| 2500 | **0.416** | 42.1% | 198 | 5.71 | 0.476 | **0.42 돌파** |
| 3000 | **0.386** | 51.0% | 177 | 5.47 | 0.456 | V38.3 최종(0.456) 대폭 하회 |
| 3500 | **0.386** | 51.2% | 182 | 5.43 | - | **새 plateau 형성** |
| 4000 | **0.384** | 51.1% | 170 | 5.18 | - | ep_len 170, 붕괴 위험 |

### V40-A 분석

**가설 A 확정: CaT 압력 부족이 0.45 plateau의 원인이었다.**

CaT prob 0.0015→0.003 단일 변수 변경만으로:
- V38.3의 0.45 한계를 **0.384까지 돌파** (-16%)
- 0.45 돌파는 iter 2000에서 달성 (V38.3보다 1000 iter 빠름)
- 0.42 판정 기준은 iter 2500에서 달성

**새로운 plateau**: 0.384~0.386 (iter 3000~4000, 1000 iter 정체)
- V38.3에서 0.45 plateau와 동일한 구조가 0.384에서 반복
- splay 51%에서 안정화 — agent가 "51% 에피소드 손실 감수" 전략
- CaT 한계는 존재하지만, prob 상향으로 한계점을 이동시킬 수 있음이 증명됨

**우려 사항**:
- **ep_len 170** — 180 임계 아래, 학습 품질 저하 중
- **splay 51%** — 에피소드 절반 이상 CaT 종료, V38.3(30%)보다 훨씬 높음
- **stride 5.18** — 하락 추세 (V38.3 iter 6000: 7.23)
- prob를 더 올리면 ep_len이 더 떨어질 위험 → **CaT prob 단독 상향의 실질적 천장**

**판정 매트릭스 결과 매핑**:

| 예상 결과 | 실제 결과 | 매핑 |
|----------|----------|------|
| dev < 0.42, ep_len > 200 | dev=0.384, ep_len=170 | **가장 가까운: "dev < 0.42, ep_len < 180"** |
| 해석 | CaT가 밀어넣었지만 ep_len 붕괴 진행 | 가설 A 지지 + 가설 B 부분 발현 |

즉 **가설 A가 주 원인이지만, 가설 B(ep_len 하락)도 부차적으로 발생**. CaT prob 0.003은 0.384까지 도달했지만, ep_len 170으로 지속 가능하지 않음.

### V40-A 교훈

1. **CaT prob 상향은 효과적** — 0.45→0.384, prob 2배만으로 -16% 달성
2. **하지만 ep_len 대가가 큼** — 250→170, splay 51%
3. **새 plateau는 prob 한계가 아닌 ep_len 한계** — splay 51%에서 에피소드가 너무 짧아 학습 불가
4. **prob를 더 올려도 ep_len이 더 떨어질 뿐** — CaT prob 단독 상향의 천장은 ~0.38
5. **0.38 이하는 CaT 외 추가 전략 필요** — positive incentive, gait 구조화, 또는 num_envs 조정

### 다음 방향 후보

- **prob 0.003 유지 + positive posture reward 추가** — 어깨 모으면 직접 보상, ep_len 회복 가능
- **prob 0.002로 중간값** — ep_len 200+ 유지하면서 dev 0.40 근처 달성 시도
- **num_envs 8192 + prob 0.003** — 탐색 다양성 증가로 plateau 돌파 시도
