...
## 8. V40-A-2: prob 0.002 sweet spot 실험 (진행 중)

### 설계

V40-A와 동일 구성, **prob 0.003→0.002만 변경.**

Trade-off 곡선 3번째 데이터 포인트:

| 실험 | prob | 예상 dev | 예상 ep_len | 예상 splay |
|------|------|---------|-----------|-----------|
| V38.3 | 0.0015 | 0.45 | 215 | 30% |
| **V40-A-2** | **0.002** | **~0.42** | **~200** | **~35%** |
| V40-A | 0.003 | 0.384 | 170 | 51% |

**기대**: dev 0.42 + ep_len 200 동시 달성 = V40 판정 기준 충족

Run: (진행 중)

---

## 9. 다음 방향 후보 (V40-A-2 이후)

| 후보 | 내용 | 기대 효과 |
|------|------|----------|
| prob 유지 + **positive posture reward** | 어깨 모으면 직접 보상 | trade-off 곡선 자체를 이동 |
| **num_envs 8192** | 탐색 다양성 증가 | entropy 유지, plateau 돌파 가능 |
| **최적 prob 산출** | 3점 곡선에서 dev/ep_len 최적점 계산 | prob fine-tuning |
