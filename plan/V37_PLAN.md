...
**작성일**: 2026-03-20
**상태**: ❌ V37 실패 (iter 600에서 보행 붕괴) → V37.2 훈련 중
...
|------|--------|--------|------|
| 400 | 181.1 | 247.6 | 부팅 성공 |
| 500 | 184.5 | 239.9 | splay_ramp 시작 직전 |
| **600** | **-92.1** | 250.0 | **급락 시작 (리스크#1 적중)** |
| 700 | -213.0 | 244.4 | 급락 지속 |
| 800 | -346.8 | 249.4 | 서있지만 reward 붕괴 |
| 1000 | -44.8 | **15.5** | **ep_len 완전 붕괴** |
| 1200 | -24.5 | 15.6 | 넘어짐 고착 |

**실패 원인**: 3가지 동시 변경(shoulder -15, stance -8, height 0.22)이 보행을 파괴. ramp 초반(20% 진행)부터 reward 급락, 이후 ep_len까지 붕괴. 교훈 #12(한 번에 하나만) 위반.

---

## V37.2 — 보수적 단일 변수 (현재 훈련 중)

### V37 → V37.2 변경

| 파라미터 | V37 | V37.2 | 근거 |
|---------|-----|-------|------|
| shoulder_neutral final | -15.0 | **-10.0** | V37 ramp 20%(-7.8쯤)에서 이미 붕괴 → -10이 상한 |
| stance_width final | -8.0 | **-3.0 (변경 없음)** | 단일 변수 원칙 |
| height target final | 0.22 | **0.23 (변경 없음)** | 단일 변수 원칙 |
| ramp 구간 | 500~1000 | **500~1500** | 2배 느린 적용 |

### V37.2 Milestone 추이
| iter | reward | ep_len | vs V37 | 비고 |
|------|--------|--------|--------|------|
| 400 | 179.0 | 245.3 | 동일 | 부팅 성공 |
| 500 | 184.5 | 239.9 | 동일 | ramp 시작 직전 |
| 600 | -1.6 | 250.0 | V37: -92 | ✅ 급락 폭 대폭 완화 |
| 700 | -51.6 | 247.6 | V37: -213 | ✅ |
| 800 | -129.4 | 247.6 | V37: -347 → 붕괴 | ✅ ep_len 유지 |
| 900 | -115.7 | 250.0 | V37: 붕괴 | ✅ 회복 시작 |
| 1000 | -91.2 | 247.6 | V37: ep_len=15 | ✅ 반등 중 |
| 1100 | -68.2 | 249.1 | | 회복 지속 |
| 1200 | -50.6 | 247.6 | | 회복 지속 |

### V37.2 중간 판정 (iter 1262)

**보행 안정성**: ✅ ep_len 247~250 유지, V37처럼 붕괴하지 않음
**reward 추이**: ✅ iter 800 바닥(-129)에서 -38까지 꾸준히 반등 중
**shoulder_dev**: ❌ 0.532 — V36(0.543) 대비 2% 감소, 사실상 변화 없음
**stride_length**: ✅ 7.29 — V36(6.08) 대비 +20% 개선

**결론**: 보행은 안정적이나 **L2 penalty -10.0으로도 splay 행동 변경 불가**. shoulder_dev가 ramp 76% 진행 시점에서도 0.53 고착. iter 2000까지 관찰 후 Barrier 함수 전환 검토.
