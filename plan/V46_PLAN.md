...
`V45`에서 정리된 건 "문제가 여러 개 섞여 있다"는 사실이었다.
그래서 `V46`은 처음부터 하나의 정답 버전이 아니라,
`3개의 독립 가설`을 순차 검증하는 실험 묶음으로 설계됐다.

### 처음 계획

```text
Run A: gait reward 부족이 원인인지 본다
Run B: shoulder-leg 분리로 trade-off가 풀리는지 본다
Run C: coupling shaping이 실제로 들어오는지 본다
```

### 이 버전의 핵심 가치

`V46`은 결과 자체보다 실험 방법론의 전환점이었다.
...
V46부터
- 한 run = 한 가설
- baseline은 V43-E처럼 가장 깨끗한 축을 선택
```

### 이후로 이어진 이유

이 3-run 전략을 통해 결국 확인한 건,
`가이트 reward 일부 복원`이나 `shoulder 분리`만으로는 `V38.3` 수준을 못 되찾는다는 점이었다.
그래서 다음 `V47`은 아예 `작동하던 rich reward 생태계로 돌아가는` 전략으로 넘어가게 된다.
