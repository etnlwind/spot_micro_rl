...
def main() -> None:
    parser = argparse.ArgumentParser(description="V2 read-only heartbeat")
    ...
    common.acquire_pid_lock(common.HEARTBEAT_PID_FILE, "v2 heartbeat", common.HEARTBEAT_LOG)
    common.write_log(f"V2 heartbeat started | iter_step={args.iter_step} | poll={args.poll}s", common.HEARTBEAT_LOG)
    ...
    try:
        while True:
            try:
                ...
                data = common.legacy_heartbeat.read_tfevents(run_dir)
                if not data:
                    time.sleep(args.poll)
                    continue
                ...
                report_text = common.legacy_heartbeat.format_report(data, run_name, cycle_num=(milestone // args.iter_step))
                record = common.legacy_heartbeat.build_report_record(data, run_name, cycle_num=(milestone // args.iter_step), report_kind="v2-heartbeat")
                common.legacy_heartbeat.append_report_record(run_dir, record)
