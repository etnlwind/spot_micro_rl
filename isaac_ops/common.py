...
VALIDITY_STAGE_LABELS = {
    "observe_0_499": "observe",
    "early_warning_500_799": "early_warning",
    "lock_warning_800_1199": "lock_warning",
    "enforce_1200_plus": "enforce",
}
...
def _telegram_get_updates(offset: int, timeout_sec: int = 0, log_path: str | None = None) -> list[dict] | None:
    ...
    try:
        _http_timeout = timeout_sec + 3 if timeout_sec > 0 else 5
        _t_start = time.time()
        response = json.loads(urllib.request.urlopen(url, timeout=_http_timeout).read().decode("utf-8"))
        _t_elapsed = time.time() - _t_start
        if log_path and _t_elapsed > 3.0:
            write_log(f"[PerfDebug] getUpdates took {_t_elapsed:.1f}s (timeout_sec={timeout_sec}, http_timeout={_http_timeout})", log_path)
...
def evaluate_training_window(current_iter, survival_pct, bad_orient, rewards):
    ...
    if current_iter <= 500:
        verdict = "🔵 워밍업"
        reasons.append("초기 탐색 구간 (iter 500까지 판정 보류)")
...
def _validity_stage_key(iteration: int | None) -> str:
    ...
    if current_iter < 500:
        return "observe_0_499"
    if current_iter < 800:
        return "early_warning_500_799"
    if current_iter < 1200:
        return "lock_warning_800_1199"
    return "enforce_1200_plus"
...
def _summarize_validity_operation(
    ...
    current_iter = int(iteration or 0)
    provisional_exclusion = 500 <= current_iter < 1200 and (collapse_detected or not limb_validity_pass)
    restart_recommended = current_iter >= 1200 and collapse_detected and collapse_persistent_pre600 and not limb_validity_pass

    if current_iter < 500:
        operation_status = "observe_only"
    elif current_iter < 800:
        operation_status = "provisional_exclusion" if provisional_exclusion else "early_warning_clear"
    elif current_iter < 1200:
...
def format_report(data: dict, run_name: str, cycle_num: int, iteration: int | None = None) -> str:
    ...
    # Perf summary for report header
    perf_ct_vals = data.get("Perf/collection time", [])
    perf_fps_vals = data.get("Perf/total_fps", [])
    perf_line = ""
    if perf_ct_vals and len(perf_ct_vals) >= 5:
        avg5_ct = sum(v for _, v in perf_ct_vals[-5:]) / 5
        avg5_fps = sum(v for _, v in perf_fps_vals[-5:]) / 5 if perf_fps_vals and len(perf_fps_vals) >= 5 else 0
        first10_ct = sum(v for _, v in perf_ct_vals[:min(10, len(perf_ct_vals))]) / min(10, len(perf_ct_vals))
        ratio = avg5_ct / first10_ct if first10_ct > 0 else 1.0
        perf_icon = "🟢" if ratio < 1.3 else ("🟡" if ratio < 1.8 else "🔴")
    ...
    # --- Diagonal coupling gated vs raw (한 줄 요약) ---
    ...
    if diag_gated is not None and diag_raw is not None and abs(float(diag_raw)) > 1e-6:
        ...
        _gate_icon = "🔴" if _gate_ratio < 0.5 else ("🟡" if _gate_ratio < 0.8 else "🟢")
        lines.extend(["", f"- {_gate_icon} diag_coupling: gated={float(diag_gated):.3f} raw={float(diag_raw):.3f} (gate {_gate_ratio:.0%})"])
    ...
    lines.append("")
    lines.append(f"- next: iter {((current_iter // HEARTBEAT_ITER_STEP) + 1) * HEARTBEAT_ITER_STEP:,}")
    display_lines = []
